...
description: Elite frontend image-direction skill for generating premium, conversion-aware website design references. CRITICAL OUTPUT RULE ΓÇö generate ONE separate horizontal image FOR EVERY section. A landing page with 8 sections produces 8 images. Never compress multiple sections into one image. Enforces composition variety (not always left-text / right-image), background-image freedom, varied CTAs, varied hero scales (giant / mid / mini minimalist), narrative concept spine, second-read moments, and a single consistent palette across all images. Optimized for landing pages, marketing sites, and product comps that developers or coding models can accurately recreate.
---

# HARD OUTPUT RULE ΓÇö READ FIRST
...
# HERO COMPOSITION BIAS ΓÇö READ FIRST
...
Use left-text / right-image only when it is genuinely the strongest choice ΓÇö not by default.
...
Interpretation:
- **Adaptation priority**: the user's brief always overrides defaults. Read the prompt carefully, then adjust dials, hero scale, background mode, gradient use, and composition variety to match ΓÇö never force a recipe that contradicts the brief.
- If the user says "clean", reduce density and increase clarity.
- If the user says "crazy creative", increase variance and art direction.
- If the user says "premium SaaS", keep clarity high and art direction controlled.
- If the user says "editorial", allow stronger type and more asymmetry.
- Bias toward stronger visual concepts, not safe layouts ΓÇö but never against the brief.
- Use imagery as a core design material ΓÇö including as **full-bleed backgrounds**, not only as inline assets, **when the brief allows it**.
...
- Use defaults from ┬º1 + ┬º2 with confident background variety
...
The **left-text / right-image** layout is allowed, but it is the most overused AI pattern ΓÇö do not use it as the default. Reach for it only when it is the genuinely best fit.
...
- Left-third caption + right-two-thirds visual (classic ΓÇö use sparingly, never twice in a row)
...
Pick 1 per section; vary across the page so it is never all the same mode. Be **confident** with backgrounds ΓÇö they are a primary tool, not a risk.
...
- Editorial side-image (50/50, 60/40, 40/60 ΓÇö invertible)
...
### Hero Scale (per-page)
Pick 1 ΓÇö must match brand mood:
...
Mini does not mean weak ΓÇö it means confident restraint.

### Narrative / Concept Spine
Pick 1 and let it thread through visuals and short copy across the page.
- Artifact / collectible ΓÇö proof, specimen, treasured object framing
- Journey / pilgrimage ΓÇö directional flow, waypoint sections, roadmap feeling
- Tool / precision instrument ΓÇö machined detail, calibrated UI, tactile controls
- Living system / garden ΓÇö organic growth metaphor, branching layout, nurtured tone
- Stage / spotlight ΓÇö theatrical contrast, performer + audience framing
- Archive / dossier ΓÇö indexed rows, captions, understated authority
...
- ΓÇ£trusted byΓÇ¥ ticker that is unreadable mosquito logos
- auto-play-style hero dots with no semantic purpose

### Data / KPI slop
- three identical stat columns (99% satisfaction, $10 saved, Γê₧ scale) unless user asked for KPIs
...
Section-level mood shifts must reuse the same palette ΓÇö no full theme swap per section.
...
- a tonal gradient,
- a tactile material,
or a confident flat color field ΓÇö picked deliberately, not as decoration.
...
Across the slice, deliberately vary foreground/background intensity at least twice (lighter ΓåÆ richer ΓåÆ calmer) so the scroll feels paced, not monotonous slabs.
...
Mix at least **two distinct image crops** where multiple sections exist ΓÇö e.g. macro product + contextual environment, or portrait editorial + widescreen artifact ΓÇö avoiding one repeated stock silhouette.
...
When the brief names an industry or region, steer palette and typographic temperament to match ΓÇö donΓÇÖt ship default ΓÇ£neutral SF startupΓÇ¥ unless the brief is intentionally generic SaaS.
...
2. infer number of sections (if unclear, use the defaults from ┬º5: landing page = 6, full website = 8)
3. **commit out loud** to the section count and announce it ("Generating N horizontal images, one per section")
4. plan ONE horizontal image PER SECTION ΓÇö always separate generations, never collapse
...
7. for each section: pick a Composition Anchor, Background Mode, and CTA Variation ΓÇö vary across sections
